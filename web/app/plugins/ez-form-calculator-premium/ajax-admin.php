...
if (!wp_verify_nonce($nonce, "ezfc-nonce")) {
	send_ajax(array("error" => __("Unable to verify security nonce. Please refresh this page."), "ezfc"));
	die();
}
...
switch ($action) {
	...
	case "form_clear":
		send_ajax($ezfc->form_clear($f_id));
	break;

	case "form_delete":
		send_ajax($ezfc->form_delete($f_id));
	break;

	case "form_delete_submissions":
		send_ajax($ezfc->form_delete_submissions($f_id));
	break;
	...
	case "form_duplicate":
		send_ajax($ezfc->form_duplicate($f_id));
	break;
	...
	case "form_import_upload":
		...
		// empty
			...
			send_ajax(array("error" => __("Empty form data or invalid file format", "ezfc")));
			die();
		}
		...
		if (!$import_data_json) {
			...
			if (!$import_data_json) {
				send_ajax(array("error" => __("Unable to import elements.", "ezfc")));
				die();
			}
		}
	...
	case "form_save_post":
		...
		if ($action == "form_save_post") {
			// send post url
			send_ajax($ezfc->form_add_post($id));
		}
		else {
			send_ajax($res);
		}
	...
	case "form_save_template":
		send_ajax($ezfc->form_save_template($f_id));
	break;
	...
	case "form_submission_delete":
		send_ajax($ezfc->form_submission_delete($f_id));
	break;
	...
	case "form_update_options":
		send_ajax($ezfc->form_update_options($f_id, $data["opt"]));
	break;
	...
	case "form_element_delete":
		...
		send_ajax($ezfc->form_element_delete($id, $child_element_ids));
	break;
