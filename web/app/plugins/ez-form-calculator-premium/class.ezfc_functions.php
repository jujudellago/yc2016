...
abstract class Ezfc_Functions {
	...
	/**
		manual plugin update with zip file
	**/
	public static function plugin_update() {
		...
		$plugin_path_update      = $plugin_path . "/update/";
		$plugin_path_update_slug = $plugin_path . "/update/" . self::$plugin_slug;

		// skip if unzip file does not exist
		if (!function_exists("unzip_file")) return self::send_message("error", __("Unfortunately, 'unzip_file' is not installed on your webserver.", "ezfc"));
		// skip if no file was uploaded
		if (count($_FILES) < 1) return false;

		if(!$wp_filesystem->is_dir($plugin_path_update)) {
			$wp_filesystem->mkdir($plugin_path_update);
		}

		$file   = $_FILES["ezfc-update"]["tmp_name"];
		$result = unzip_file($file, $plugin_path_update); 

		if ($result !== true) {
			return self::send_message("error", __("Update has failed: unable to unzip files.", "ezfc"));
		}

		// check if directory exists
		if (!$wp_filesystem->is_dir($plugin_path_update_slug)) {
			return self::send_message("error", __("Update has failed: update directory does not exist.", "ezfc"));
		}

		// copy dir
		self::recurse_copy($plugin_path_update_slug, $plugin_dir);

		// remove update files
		if (!self::delete_dir($plugin_path_update)) {
			return self::send_message("notification", __("Notice: unable to remove update directory.<br>", "ezfc"));
		}

		return self::send_message("success", __("Update successful!", "ezfc"));
	}
