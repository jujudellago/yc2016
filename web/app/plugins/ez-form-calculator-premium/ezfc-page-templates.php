...
if (!empty($_REQUEST["template_install_id"])) {
	$res = EZFC_Functions::template_install($_REQUEST["template_install_id"]);
	$message = array_shift($res);
}
else if (!empty($_REQUEST["template_submit_id"])) {
	$res = EZFC_Functions::template_submit($_REQUEST);
	$message = array_shift($res);
}
	...
	<div id="tabs">
			...
			<div class="container-fluid">
				<div class="row">
					<form method="POST" name="ezfc-form" class="ezfc-form" action="<?php echo admin_url('admin.php'); ?>?page=ezfc-templates" novalidate>
						...
						<?php
						if (count($templates_browse) > 0) {
							foreach ($templates_browse as $template) {
		...
		<div id="tab-1">
			<div class="container">
				<div class="row">
					<p>
						<?php echo __("You can submit your own templates to share with the community. We will review the template and if it fits for public purpose, we will add it to the list."); ?>
					</p>

					<p>
						<?php echo __("Please note that we will not expose any personal data."); ?>
					</p>
